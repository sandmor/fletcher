...
- **Background compositing** — add a solid color or uploaded image background to cutouts in the image studio
...
1. User picks an image in the studio → `getBackgroundUploadUrl` (Convex action, job ownership verified)
2. Client uploads directly to S3 via presigned PUT
3. `updateJobBackground` action saves `{ type: "image", imageUrl, fileName }` on the job

Solid color changes use the same `updateJobBackground` action.
